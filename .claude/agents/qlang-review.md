...
- A leading combinator (captured in `Pipeline.leadingCombinator`, one of `!|` / `|` / `*` / `>>`) is the first-step form: it routes the first operand step of a sub-pipeline through that combinator even though there is no preceding step. The `!|` form is used inside `filter(…)` / `when(…)` / `if(…)` lambdas where the per-element pipeValue may be on either track. A plain comment in head position hands the head to the first operand step; the follower's continuation unit carries the absorbed marker (`combinator: null`) unless the author wrote a combinator after the comment.
