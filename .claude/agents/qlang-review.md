...
- Per-site errors are asserted by class name (`expect(e.name).toBe('FilterSubjectNotVec')`) AND by `instanceof QlangTypeError` AND by structured context fields (`expect(e.context.position).toBe(2)`). All three.
...
- Coverage must meet the thresholds in `vitest.config.mjs` (currently 99/97/99/99 for statements/branches/functions/lines). If a new file dips below, that's a finding.
