...
// declaration with the name it is bound under, `:name`, in the order
// of the names. `manifest` answers the tree of names [D62], so a test
// that reads every declaration reads the environment itself.

import { isRuntimeKey, isTagBindingName } from '../../src/env-keys.mjs';
import { isQMap, TAG_HEADER_SYMBOL, stampTagHeader } from '../../src/types.mjs';
...
export function catalogEntriesOf(env, { tags }) {
  const entries = [];
  for (const [name, declaration] of env) {
