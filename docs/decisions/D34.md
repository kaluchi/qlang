...
Replaced in part by [D62], under which the walk ends at `::qlang/any`,
beneath the core's kinds, where the verbs of any value live.
Replaced in part by [D67], under which `sort` over a tag stacked on a set
answers the bare vector, and a contract that several kinds answer lives
on its provider's `any`.
