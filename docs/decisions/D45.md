...
Replaced in part by [D67], under which a verb's slots are the leading
declarations of its quote, its subject given by its module.
Replaced by [D67] and [D70], under which a slot is a leading declaration
with its kind or a default, a name with neither is no slot, the subject
comes from the verb's module and the result from `:returns`; the short
spelling of a kind of the core in a declaration stands by [D32].
