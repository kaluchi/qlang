...
**I.3.2. Conduit в Map-value** — printValue выдаёт `def(:n,
...
doc-segment типы через `def(::warn, ...)`,
`def(::link, ...)`, `def(::note, ...)` без grammar-уровневых
...
- `{} | use`, `def(:counter, {})`, `def(:bag, [])` — типичные
...
```
qlang 'def(:safe, @hostOp)' → !{:thrown :EffectLaunderingAtLetParse}
qlang 'def(:@safe, @hostOp)' → ok
```
...
- **Effect-decoration через TaggedLit payload.** `qlang
  'def(:safeOp, ::conduit[[] \`@hostCall\`])'` → error
...
- **Effect-laundering safety net по-прежнему loud-fail.**
  `qlang 'def(:safe, @hostOp)'` → `:EffectLaunderingAtLetParse`.
