...
- `errors.mjs` — `QlangError` base hierarchy (`.location`,
  ...
  `EffectLaunderingAtCallError`, `UnresolvedIdentifierError`,
  `DivisionByZeroError`.
...
- `primitives.mjs` — `createPrimitiveRegistry()` / module-level
  ...
  per-site invariant errors `PrimitiveKeyNotStringError` /
  `PrimitiveKeyAlreadyBoundError` / `PrimitiveRegistrySealedError`,
  dispatch-time data error `PrimitiveKeyUnboundError`).
...
**I.3.2. Conduit в Map-value** — printValue выдаёт `:n body` step-form. Это declaration-step, не value-form. Parse
...
3. Lookup `::tag` в type-namespace env. Если binding
   отсутствует — `TaggedLitTagNotFoundError` error.
...
```qlang
::positiveInt {:constructor ::conduit[[:n]
  `n | when(lt(0), error({:kind :NegativeNotAllowed})) | n`]}

::age {:constructor ::conduit[[:n]
...
- **Pure** в смысле no-side-effects on env / no I/O.
  ...
  декларироваться через `@`-prefixed tag (`::@launch ...`).
  Effect-laundering invariant работает на type-level
  symmetrically с let/def: `::tag {:constructor
...
Doc-content уже парсится parse-time в `:segments` Vec из
`:Prose` / `:Quote` / TaggedLit-built сегментов (§II.5).
...
```qlang
::lazy {:constructor ::conduit[[:wrapped]
  `wrapped`]}  -- noop, просто хранит Quote

::cond {:constructor ::conduit[[:branches]
...
**Target Doc-content canon — TaggedLit-driven сегменты.**
Doc-value хранит `:segments` Vec структурных кусков; tokenizer
распознаёт три формы:

- `` `code` `` → Quote-value segment (тот же value-class что
  II.2, embedded в Doc-content как frozen code-as-data fragment).
- `::tag[payload]` (или другая Primary-форма) → TaggedLit
  segment; eval через `::tag` constructor возвращает tagged
  value (например `::assertion[\`snippet\` \`expected\`]` →
  Assertion-value `{:qlang/kind :assertion :snippet ... :expected ...}`).
- Plain text между ними → Prose segment.

`:Assertion` теперь **частный случай** TaggedLit'а — built-in
`::assertion` type-binding с constructor'ом, принимающим
Vec из двух Quote'ов. `runExamples` фильтрует segments по
`:qlang/kind | eq(:assertion)`, evaluates `:snippet`,
сравнивает с `:expected`. Author может определять свои
doc-segment типы через `::warn ...`,
`::link ...`, `::note ...` без grammar-уровневых
изменений — uniform pattern, composable, explicit type
identifier видно в source.

Остальное (keyword references, имена в prose) — Prose
plain-text'ом. Модель распознаёт keywords plain-text'ом без
grammar-уровневой дискриминации.
...
`:name ...` в captured-form — имя первый captured-arg.
...
**Type definitions через `::tag descriptor`** (II.3).
...
!{
  :origin :qlang/eval
  :kind :type-error
  :thrown :SumElementNotNumberError
...
!{ ... :thrown :GtOperandsNotComparableError
...
qlang '{"k":1} | use'                    → !{:thrown :UseSubjectNotMapError ...}
...
```
qlang '|~~ doc ~~| :x 42 | reify(:x) | /docs | first' → "doc"
...
```
qlang ':foo 42 | ::foo {:qlang/kind :type ...} | :foo'  → 42
qlang ':foo 42 | ::foo {:qlang/kind :type ...} | ::foo | reify | /kind' → :type
```

**Effect-laundering safety net по-прежнему живой** (§I.2 invariant):

```
qlang ':safe @hostOp' → !{:thrown :EffectLaunderingAtLetParse}
qlang ':@safe @hostOp' → ok
```
...
```
qlang '::launchMissile {:constructor ::conduit[[:p] `p | @nukeApi`]}'
...
- Doc-content **сегментация** (`:Prose` / `:Quote` /
  TaggedLit-built сегменты), `runExamples` assertion-extraction
  через `::assertion` segments, axis-навигация — всё
  откладывается до Phase 8.
...
- Новый builtin `def`. Captured-form `:name body` /
  `:name [:p] body` / `def(:name)` с attached doc.
...
- **2-arg conduit path.** `qlang ':double mul(2) | 5 |
  double'` → `10`. Body `mul(2)` содержит OperandCall →
  conduit → invoke per-lookup.
- **2-arg snapshot path.** `qlang ':pi 3.14 | :pi'` →
  `3.14`. Body `3.14` — pure literal → snapshot → eval at
  def-time, бинд-значение.
- **2-arg conduit с partial application.** `qlang ':next add(1) | 5 | next'` → `6`.
- **3-arg parametric.** `qlang ':@surround [:pfx :sfx] (prepend(pfx) | append(sfx)) | "x" | @surround("[", "]")'`
...
- **Pipeline-transparent.** `qlang '42 | :x 99'` → `42`
  (pipeValue не меняется, declaration consumes только
  captured-args).
- **Purity-criterion granularity.** `qlang ':onlyLits [1 2 {:k :v}] | :onlyLits'` → `[1 2 {:k :v}]` (snapshot,
  все литералы). `qlang ':withCall [1 2 (mul(2))] | 5 |
...
- Переписать `core/lib/qlang/core.qlang` из одного Map literal'а
  в серию `def`-step'ов.
- Каждый builtin — отдельный `:name descriptor`.
...
- `::tag descriptor` для type definitions; descriptor —
...
- TaggedLit eval (§II.3.1):
  ...
  2. Lookup `::tag` в type-namespace env (`TaggedLitTagNotFoundError`
     если absent).
...
- **TaggedLit construction.** `qlang '::duration {:qlang/kind :type :qlang/impl :qlang/prim/duration ...} |
  ::duration{:hours 3} | /hours'` → `3` (constructor получает
  payload и возвращает Map с поле `:hours`).
- **Two-namespace coexist.** `qlang ':foo 42 |
  ::foo {:qlang/kind :type ...} | [:foo ::foo]'` →
...
### Phase 8 — Axis-операнды + Doc-content segmentation

**Что:**

- Doc-content tokenizer (parse-time или lazy on `/segments`):
  - `` `code` `` → Quote-value segment.
  - `::tag[payload]` (или другая Primary-форма) → TaggedLit
    segment; eval через `::tag` constructor.
  - Plain text между ними → Prose segment
    (`{:qlang/kind :prose :text "..."}`).
- Built-in `::assertion` type-binding (constructor для assertion
  segment): принимает Vec из двух Quote'ов
  `[\`snippet\` \`expected\`]`, возвращает Assertion-value
  (`{:qlang/kind :assertion :snippet <Quote> :expected <Quote>}`).
- Doc-value's `:segments` projection lazy-parses content на
  первом доступе.
...
- Реализация — ходят по `qlang/ast/<uri>` Quote'ам, ищут
  binding by name, читают `.docs[]` strings → parse в Doc-value
  с `:segments`.
- Multi-source aggregation в `docs` operand'е.
- `runExamples` фильтрует Doc segments по
  `:qlang/kind | eq(:assertion)`, evaluates `:snippet`,
  сравнивает с `:expected`.
- `reify` обновлён под новую structure (`:docs` теперь Doc-value
  или Vec Doc-values, не raw string Vec).

**Зависимости:** Phase 7 (Module Quote в env), Phase 6
(TaggedLit + ::tag constructors).

**Verify:**

- **Doc-content segmentation.**
  `qlang '|~~ See \`mul(2)\` for ref. ~~| | /segments | count'` →
  3 (Prose, Quote, Prose).
- **TaggedLit segment.**
  `qlang '|~~ ::assertion[\`[1 2 3] | count\` \`3\`] ~~| | /segments | filter(/qlang/kind | eq(:assertion)) | count'` → 1.
- **Value-level navigation.** `qlang ':filter | docs'` →
  Doc-value (или Vec Doc-values при multi-source).
...
- **Examples axis.** `qlang ':count | docs | /segments | filter(/qlang/kind | eq(:assertion)) | first | /snippet | eval | eq(/expected | eval)'` — assertion segment evaluates корректно.
- **Multi-source aggregation.** Если `:keyword` documented в
  двух модулях (`core.qlang` + `domain/foo.qlang`):
  `:keyword | docs | count` → `2`.
- **reify обновлён.** `qlang 'reify(:filter) | /docs'` —
  возвращает Doc-value, не raw string Vec.
...
- core.qlang — каждый builtin descriptor содержит attached
  doc-comment с inline assertions через `::assertion[\`snippet\`
  \`expected\`]` TaggedLit segments в Doc-content.
- Поля `:examples`, `:returns`, `:modifiers` пересмотреть
  (remove или migrate в Doc-content).
- Vocabulary entries (`:CountSubjectNotContainerError`, etc.) —
...
- **`core/src/session.mjs`** — расширить `serializeSession`
  ...
  type-binding'ов (созданных через `::tag descriptor`
...
- **Effect-laundering safety net по-прежнему loud-fail.**
  `qlang ':safe @hostOp'` → `:EffectLaunderingAtLetParse`.
