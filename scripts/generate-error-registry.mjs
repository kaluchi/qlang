...
// section.
//
// Constructors (Quote-impl validators per §II.3 of the redesign plan)
// land in a separate pass — this file's purpose is to make every
// referenced tag known to env so `evalBareTypeKeyword` finds it and
// `!{:thrown ::Tag …}` literals round-trip cleanly.
...
const PROSE_OVERRIDES = {
  ...
  ProjectionSubjectNotMap: 'Projection `/key` requires a Map, Vec, or value-class subject; null subject and other shapes lift to this error.',
  ProjectionKeyNotInMap: 'Projection `/key` — the named key is absent from the Map subject (strict-miss). Use `at(:key)` for null-coalescing access.',
  ProjectionIndexOutOfBounds: 'Projection `/n` — the integer index resolves outside the Vec subject\'s `[0, length)` range.',
  ProjectionVecKeyNotInteger: 'Projection segment is a non-numeric string but the subject is a Vec — Vec indices must be integer offsets.',
  ProjectionFieldNotOnValueClass: 'Projection segment is not a publicly-projectable field on the value-class subject (Quote / Doc / …).',

  TaggedLitTagNotFound: '`::tag` constructor invocation failed — the tag is not a registered type-binding in env.',
  TaggedLitNotType: '`::tag` constructor invocation failed — the env binding under `::tag` is not a type-binding descriptor Map.',
  TaggedLitImplNotResolvable: '`::tag` constructor invocation failed — the type-binding\'s `:qlang/impl` is neither a primitive Keyword nor a Quote.',

  ApplyToNonFunction: 'Identifier resolves to a non-function value — captured arguments cannot be applied.',
  AsNameNotKeyword: '`as` requires a Keyword captured-arg (the binding name).',

  ConduitArityMismatch: 'Conduit invocation supplied a captured-arg count different from the conduit\'s declared parameter list.',
  ConduitParameterNoCapturedArgs: 'Conduit-parameter proxies are nullary — passing captured arguments to a parameter reference raises this error.',
  ConduitArityInvalid: '`::conduit[…]` payload Vec must have either 2 elements (`[params body]`) or 3 elements (`[:self params body]`).',
  ConduitSelfNameNotKeyword: '`::conduit` self-name (3-element payload form) must be a Keyword.',
  ConduitParamsNotVec: '`::conduit` params slot must be a Vec of Keywords.',
  ConduitParamNotKeyword: '`::conduit` params Vec must contain only Keywords; one element is some other type.',
  ConduitBodyNotQuote: '`::conduit` body slot must be a Quote-value carrying the body source.',
  ConduitPayloadNotVec: '`::conduit[…]` payload must be a Vec; a non-Vec payload raises this error.',
  ConduitBodyMissingSource: '`makeConduit` refuses a body AST that has no `.text` source slice — `printValue` round-trip would otherwise emit a non-parseable placeholder.',

  EffectLaunderingAtBindStepParse: 'BindStep declaration body references an `@`-effectful identifier, but the binding name itself is not `@`-prefixed — laundering caught at parse time.',
  EffectLaunderingAtCall: 'Identifier resolves to an `@`-effectful function, but the lookup name is not `@`-prefixed — laundering caught at the call site.',

  AxisBindingNotFound: 'Axis-operand (`source` / `docs` / `examples`) could not find a `BindStep` for the requested binding name across loaded modules.',
  SourceSubjectNotKeywordOrType: '`source` requires a Keyword (`:foo`) or TagKeyword (`::Foo`) subject.',
  DocsSubjectNotKeywordOrType: '`docs` requires a Keyword (`:foo`) or TagKeyword (`::Foo`) subject.',
  ExamplesSubjectNotKeywordOrType: '`examples` requires a Keyword (`:foo`) or TagKeyword (`::Foo`) subject.',
  RunExamplesSubjectShapeError: '`runExamples` requires a Keyword (binding name) or a descriptor Map carrying a `:name` String.',

  ReifyArityOverflow: '`reify` accepts 0 or 1 captured arguments, not more.',
  ReifyKeyNotKeyword: '`reify(:name)` requires a Keyword captured-arg (the binding name to look up).',

  UseSubjectNotMap: '`use` (bare form) requires a Map subject — its entries become the merged bindings.',
  UseNamespaceNotKeyword: '`use(:ns)` requires a Keyword captured-arg naming the namespace to import.',
  UseNamespaceNotFound: '`use(:ns)` — namespace not in env and no host-provided locator can resolve it.',
  UseNamespaceNotMap: '`use(:ns)` — env binding under `:ns` is not a Map; cannot import as a namespace.',
  UseNamespaceElementNotKeyword: '`use([:ns1 :ns2 …])` — every element of the namespace Vec must be a Keyword.',
  UseNamespaceCollision: '`use(#{:ns1 :ns2 …})` — two namespaces export a name under the same identifier; Set-form rejects collisions.',
  UseNameNotExported: '`use(:ns, #{:name1 …})` — selection filter names an identifier the namespace does not export.',

  CoalesceNoAlternatives: '`coalesce` requires at least one captured alternative sub-pipeline.',
  FirstTruthyNoAlternatives: '`firstTruthy` requires at least one captured alternative sub-pipeline.',
  CondNoBranches: '`cond` requires at least one (predicate, branch) pair plus an optional trailing default.',

  FilterVecOrSetPredArityInvalid: '`filter` over Vec or Set requires a predicate conduit with 0 or 1 params (`[:x]` binds the element). 2 params (`[:k :v]`) only meaningful on Map.',
  FilterMapPredArityInvalid: '`filter` over Map requires a predicate conduit with 0, 1, or 2 params (`[:k :v]` binds key + value).',
  EveryVecOrSetPredArityInvalid: '`every` over Vec or Set requires a predicate conduit with 0 or 1 params (`[:x]` binds the element). 2 params (`[:k :v]`) only meaningful on Map.',
  EveryMapPredArityInvalid: '`every` over Map requires a predicate conduit with 0, 1, or 2 params (`[:k :v]` binds key + value).',
  AnyVecOrSetPredArityInvalid: '`any` over Vec or Set requires a predicate conduit with 0 or 1 params (`[:x]` binds the element). 2 params (`[:k :v]`) only meaningful on Map.',
  AnyMapPredArityInvalid: '`any` over Map requires a predicate conduit with 0, 1, or 2 params (`[:k :v]` binds key + value).',

  GroupByKeyNotKeyword: '`groupBy(/key)` — the key sub-pipeline must produce a Keyword for every Vec element; one element produced a non-keyword.',
  IndexByKeyNotKeyword: '`indexBy(/key)` — the key sub-pipeline must produce a Keyword for every Vec element; one element produced a non-keyword.',

  SortWithCmpResultNotNumber: '`sortWith` comparator must return a Number; one comparison produced a non-Number.',
  AscPairNotMap: '`asc(/key)` comparator subject must be a pair Map (`{ :left x :right y }`).',
  DescPairNotMap: '`desc(/key)` comparator subject must be a pair Map (`{ :left x :right y }`).',
  NullsFirstPairNotMap: '`nullsFirst(/key)` comparator subject must be a pair Map (`{ :left x :right y }`).',
  NullsLastPairNotMap: '`nullsLast(/key)` comparator subject must be a pair Map (`{ :left x :right y }`).',

  UnionBareEmpty: '`union` (bare form) requires a non-empty Vec of operands to fold.',
  MinusBareEmpty: '`minus` (bare form) requires a non-empty Vec of operands to fold.',
  InterBareEmpty: '`inter` (bare form) requires a non-empty Vec of operands to fold.',
  UnionPairIncompatible: '`union` pair operands have incompatible container types — Set+Set or Map+Map only.',
  MinusPairIncompatible: '`minus` pair operands have incompatible container types — Set+Set, Map+Map, or Map+Set only.',
  InterPairIncompatible: '`inter` pair operands have incompatible container types — Set+Set, Map+Map, or Map+Set only.',
  ...
  PrimitiveKeyNotString: 'Primitive registry — bind() received a non-string handle.',
  PrimitiveKeyAlreadyBound: 'Primitive registry — two runtime modules tried to bind the same primitive name.',
  PrimitiveRegistrySealed: 'Primitive registry — bind() called after the registry was sealed at langRuntime bootstrap.',
  PrimitiveKeyUnbound: 'Primitive registry — `:qlang/impl` handle on a descriptor Map points to a primitive that was never bound.',
  ...
  FunctionValueLeakedToPrint: '`printValue` / `toPlain` invariant — a raw function value reached render. Wrap host operands in a descriptor Map carrying `:qlang/kind :builtin` and `:qlang/impl` instead of binding the function via `session.bind`.',

  Rule10ArityOverflow: 'Rule 10 — operand call supplied more captured arguments than the operand\'s declared maximum arity.',
  ValueOpArityMismatch: 'Dispatch invariant — `valueOp` impl received an unexpected captured-arg count for its declared arity.',
  HigherOrderOpArityMismatch: 'Dispatch invariant — `higherOrderOp` impl received an unexpected captured-arg count.',
  NullaryOpArgsProvided: 'Dispatch invariant — nullary operand received captured arguments.',
  OverloadedOpUnsupportedArity: 'Dispatch invariant — overloaded operand has no impl for the supplied captured-arg count.',
  StateOpArityMismatch: 'Dispatch invariant — `stateOp` impl received an unexpected captured-arg count.',
  StateOpVariadicMissingCaptured: 'Dispatch invariant — `stateOpVariadic` registration is missing the `captured` range argument.',
  HigherOrderOpVariadicMissingCaptured: 'Dispatch invariant — `higherOrderOpVariadic` registration is missing the `captured` range argument.',

  DistributeSubjectNotVec: 'Distribute combinator (`*`) — subject must be a Vec for per-element forking.',
  MergeSubjectNotVec: 'Merge combinator (`>>`) — subject must be a Vec for one-level flattening.',

  KeywordSubjectNotStringOrKeyword: '`keyword` involution requires a String or Keyword subject.',

  EvalSubjectNotMapOrQuote: '`eval` requires a Map (AST-Map) or Quote (source-form) subject.',
  ParseSubjectNotStringOrQuote: '`parse` requires a String or Quote subject.',

  ErrorDescriptorNotMap: '`error` lift requires a Map descriptor (the error\'s payload).'
};
...
const OPERAND_OVERRIDES = {
  DistributeSubjectNotVec: '*',
  MergeSubjectNotVec: '>>',
  ProjectionSubjectNotMap: '/',
  ProjectionKeyNotInMap: '/',
  ProjectionIndexOutOfBounds: '/',
  ProjectionVecKeyNotInteger: '/',
  ProjectionFieldNotOnValueClass: '/',
  TaggedLitTagNotFound: '::tag',
  TaggedLitNotType: '::tag',
  TaggedLitImplNotResolvable: '::tag',
  ApplyToNonFunction: 'identifier-lookup',
  ConduitArityMismatch: 'conduit-call',
  ConduitParameterNoCapturedArgs: 'conduit-parameter',
  EffectLaunderingAtBindStepParse: 'BindStep',
  EffectLaunderingAtCall: 'identifier-lookup',
  Rule10ArityOverflow: 'rule10',
  ValueOpArityMismatch: 'dispatch',
  HigherOrderOpArityMismatch: 'dispatch',
  NullaryOpArgsProvided: 'dispatch',
  OverloadedOpUnsupportedArity: 'dispatch',
  StateOpArityMismatch: 'dispatch',
  StateOpVariadicMissingCaptured: 'dispatch',
  HigherOrderOpVariadicMissingCaptured: 'dispatch',
  PrimitiveKeyNotString: 'primitive-registry',
  PrimitiveKeyAlreadyBound: 'primitive-registry',
  PrimitiveRegistrySealed: 'primitive-registry',
  PrimitiveKeyUnbound: 'primitive-registry',
  ...
  FunctionValueLeakedToPrint: 'print-invariant',
  ParseError: 'parse',
  ConduitBodyMissingSource: 'conduit-mint',
  AxisBindingNotFound: 'axis',
  RunExamplesSubjectShapeError: 'runExamples',
  ReifyArityOverflow: 'reify',
  ReifyKeyNotKeyword: 'reify',
...
// Sort keys: operand name first (so siblings cluster — AddLeftNotNumber
// next to AddRightNotNumber), then tag name within an operand.
...
let out = `|~ Catalog of named-error type-bindings — one entry per JS-side
   ...
   Sections are grouped by the originating operand's
   \`:category\` (matching the catalog ordering of core.qlang)
   plus a tail of runtime-invariant sections (projection,
   combinator track-dispatch, parse, codec, dispatch, registry,
   session, print). Generated by \`scripts/generate-error-registry.mjs\`.
